...
def _is_pl2303(path: str) -> bool:
    ...
    try:
        ...
        pass
    return "prolific" in str(path).lower()


def open_serial_compat(path: str, baud: int, timeout: float, write_timeout: float = 2.0):
    """Open USB-UART without PL2303 DTR/tcflush operations that can stall on Jetson."""
    if serial is None:
        raise RuntimeError("pyserial required: python -m pip install pyserial")
    if not _is_pl2303(path):
        return serial.Serial(path,baud,timeout=timeout,write_timeout=write_timeout,exclusive=True)

    import termios
    ser=serial.Serial(port=None,baudrate=baud,timeout=timeout,write_timeout=write_timeout,
                      exclusive=True,rtscts=True,dsrdtr=True)
    ser.port=path
    ser.open()
    attrs=termios.tcgetattr(ser.fileno())
    attrs[2] &= ~getattr(termios,"CRTSCTS",0)
    termios.tcsetattr(ser.fileno(),termios.TCSANOW,attrs)
    ser._hb_skip_buffer_reset=True
    print(f"[SERIAL] PL2303 compatibility mode {path} @ {baud}",flush=True)
    return ser
...
def _probe_f103_uart(path: str, baud: int = DEFAULT_BAUD, timeout: float = 0.01):
    ...
    ser=open_serial_compat(path,baud,max(0.002,timeout),2)
    try:
        if not getattr(ser,"_hb_skip_buffer_reset",False):
            try: ser.reset_input_buffer(); ser.reset_output_buffer()
            except Exception: pass
        dec=PacketDecoder()
        packet=frame(bytes((COMM_FW_VERSION,)))
        written=ser.write(packet)
        if written!=len(packet):
            raise IOError(f"short probe write {written}/{len(packet)}")
        # PL2303 tcdrain() can block indefinitely even after the complete frame
        # has entered the kernel USB-UART queue.  Do not let target discovery
        # deadlock on that adapter; transaction timeout/readback still proves
        # whether the frame actually reached the F103.
        if not getattr(ser,"_hb_skip_buffer_reset",False):
            ser.flush()
...
class VescDual:
    ...
    def flush_tx(self) -> None:
        """Drain host TX where safe; PL2303 uses reply/readback instead.

        Some Linux PL2303 combinations can block indefinitely in tcdrain().
        Every request/reply transaction already has a monotonic deadline, so
        skipping tcdrain on that adapter is safer than hanging serial ownership.
        """
    ...
    def send(self, payload: bytes):
        ...
        with lock:
            ...
            # Standard adapters may use tcdrain() here.  PL2303 compatibility
            # sessions intentionally skip blocking flush because the Linux
            # driver can remain in tty_wait_until_sent indefinitely.  A complete
            # write plus the bounded reply timeout still gives fail-closed
    ...
    def steering_calibration(self) -> dict[str, int | float | bool | str]:
        ...
        # datatypes.h: FOC_SENSOR_MODE_ENCODER=1, HALL=2, ENCODER_AB=9.
        role='encoder' if sensor_port==1 and foc_sensor in (1,9) else ('hall' if sensor_port==0 and foc_sensor==2 else 'unsupported')
