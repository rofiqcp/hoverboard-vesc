...
model=text('tools/qualify_motor_model.py'); tuner=text('tools/tune_foc_staged.py')
adc=text('tools/profile_adc_envelope.py'); campaign=text('tools/tuning_campaign_10x5.py')
ctrl=text('tools/qualify_control_modes.py'); posd=text('tools/qualify_position_process_d.py')
stlink=text('tools/pio_stlink_upload.py'); guard=text('tools/stlink_target_guard.py')
...
assert 'kpq=q(kp,1536),kpd=q(kp,1536)' in tuner and 'kiq=q(ki,4.608),kid=q(ki,4.608)' in tuner
...
assert "d32(p0['dma_tc_pending_exit'],prof['dma_tc_pending_exit'])" in tuner, 'tuner must gate on monotonic DMA delta'
...
assert 'under_reset' not in stlink and 'connect_assert_srst' not in stlink
assert 'return "swd"' in guard, 'modern ST-Link transport resolver must use swd'
...
print('STAGE2_PRODUCTION_GATE_PASS trace_authority=1 model_repeatability=1 qualification_only=1 normal_swd_only=1 process_d_observable=1')
