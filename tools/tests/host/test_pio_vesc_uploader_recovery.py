...
class Fake:
    def __init__(self,old):
        self.args=types.SimpleNamespace(transport='serial',serial_port='/dev/mock',baud=921600,boot_baud=921600,fault_stop_after=0)
...
def fake_wait(l,label,timeout=65):
    raise RuntimeError('intentional candidate boot failure')
upl._wait_application=fake_wait
try:
    try: upl.upload(link,cand); raise AssertionError('expected direct-install exception')
    except RuntimeError as e: assert 'candidate install failed' in str(e),e
finally:
    upl._wait_application=orig_wait
assert link.installs==1 and link.active==cand
print(f'PIO_VESC_UPLOADER_RECOVERY_PASS direct_uart=1 installs={link.installs} host_rollback=0 resident_bootloader_authority=1')
