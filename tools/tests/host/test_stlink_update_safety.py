...
assert '--rescue-under-reset' in u
...
assert 'under_reset: bool = False' in g
...
normal=m.openocd_program_cmd(Path('/x/openocd'),Path('/x/scripts'),'swd',Path('/tmp/app.bin'),0x08002800,100,False,Path('/tmp/meta.bin'),0x0803E800)
rescue=m.openocd_program_cmd(Path('/x/openocd'),Path('/x/scripts'),'swd',Path('/tmp/app.bin'),0x08002800,100,True,Path('/tmp/meta.bin'),0x0803E800)
assert not any('connect_assert_srst' in x for x in normal)
assert any('connect_assert_srst' in x for x in rescue)
assert any('0x0803E800' in x and 'meta.bin' in x for x in normal)

print('STLINK_UPDATE_SAFETY_PASS normal_primary=1 under_reset_explicit=1 postrun_normal_probe=3 app_vtor_pc_guard=1 confirmed_meta=1 vector_guard=1 partition_guard=1')
